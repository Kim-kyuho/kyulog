...
export default function AuthStatus() {
  ...
  if (!session?.user) {
    return (
      <button
        ...
        className="flex items-center gap-2 px-4 py-2 bg-gray-800 text-white rounded-sm hover:bg-gray-700 active:scale-95 transition-transform"
      >
  ...
  return (
    <div className="flex items-center gap-3">
      ...
      <button
        onClick={() => signOut()}
        className="px-4 py-2 bg-red-200 text-red-800 rounded-sm hover:bg-red-300 active:scale-95 transition-transform"
      >
        Logout
      </button>
    </div>
